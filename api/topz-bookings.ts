...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json(rows.map(r => ({ ...r, totalAmount: Number(r.totalAmount), advancePaid: Number(r.advancePaid), supplier: r.supplier ?? undefined })))
  }

  if (req.method === 'POST') {
    const b = req.body
    await sql(
      `INSERT INTO topz_bookings (id,createdAt,quoteNo,clientName,clientPhone,vehicleName,pickupDate,dropDate,pickupLocation,dropLocation,totalAmount,advancePaid,status,notes,tripType,supplier)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
       ON CONFLICT(id) DO UPDATE SET quoteNo=excluded.quoteNo,clientName=excluded.clientName,clientPhone=excluded.clientPhone,vehicleName=excluded.vehicleName,pickupDate=excluded.pickupDate,dropDate=excluded.dropDate,pickupLocation=excluded.pickupLocation,dropLocation=excluded.dropLocation,totalAmount=excluded.totalAmount,advancePaid=excluded.advancePaid,status=excluded.status,notes=excluded.notes,tripType=excluded.tripType,supplier=excluded.supplier`,
      [b.id, new Date().toISOString(), b.quoteNo ?? '', b.clientName ?? '', b.clientPhone ?? '', b.vehicleName ?? '', b.pickupDate ?? '', b.dropDate ?? '', b.pickupLocation ?? '', b.dropLocation ?? '', b.totalAmount ?? 0, b.advancePaid ?? 0, b.status ?? 'confirmed', b.notes ?? '', b.tripType ?? 'outstation', b.supplier ?? null]
    )
