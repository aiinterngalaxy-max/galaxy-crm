...
async function init() {
  await fetch(`${TURSO_URL}/v2/pipeline`, {
    ...
    body: JSON.stringify({
      requests: [
        { type: 'execute', stmt: { sql: `CREATE TABLE IF NOT EXISTS topz_bookings (id TEXT PRIMARY KEY, createdAt TEXT NOT NULL, quoteNo TEXT NOT NULL DEFAULT '', clientName TEXT NOT NULL DEFAULT '', clientPhone TEXT NOT NULL DEFAULT '', vehicleName TEXT NOT NULL DEFAULT '', pickupDate TEXT NOT NULL DEFAULT '', dropDate TEXT NOT NULL DEFAULT '', pickupLocation TEXT NOT NULL DEFAULT '', dropLocation TEXT NOT NULL DEFAULT '', totalAmount REAL NOT NULL DEFAULT 0, advancePaid REAL NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'confirmed', notes TEXT NOT NULL DEFAULT '', tripType TEXT NOT NULL DEFAULT 'outstation', supplier TEXT, commission REAL NOT NULL DEFAULT 0, takenBy TEXT)`, args: [] } },
        // Migrations for pre-existing tables: fail silently (duplicate column) once the column exists — this handler ignores response errors.
        { type: 'execute', stmt: { sql: `ALTER TABLE topz_bookings ADD COLUMN commission REAL NOT NULL DEFAULT 0`, args: [] } },
        { type: 'execute', stmt: { sql: `ALTER TABLE topz_bookings ADD COLUMN takenBy TEXT`, args: [] } },
        { type: 'close' },
      ],
    }),
  })
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json(rows.map(r => ({ ...r, totalAmount: Number(r.totalAmount), advancePaid: Number(r.advancePaid), commission: Number(r.commission ?? 0), supplier: r.supplier ?? undefined, takenBy: r.takenBy ?? undefined })))
  }

  if (req.method === 'POST') {
    const b = req.body
    await sql(
      `INSERT INTO topz_bookings (id,createdAt,quoteNo,clientName,clientPhone,vehicleName,pickupDate,dropDate,pickupLocation,dropLocation,totalAmount,advancePaid,status,notes,tripType,supplier,commission,takenBy)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
       ON CONFLICT(id) DO UPDATE SET quoteNo=excluded.quoteNo,clientName=excluded.clientName,clientPhone=excluded.clientPhone,vehicleName=excluded.vehicleName,pickupDate=excluded.pickupDate,dropDate=excluded.dropDate,pickupLocation=excluded.pickupLocation,dropLocation=excluded.dropLocation,totalAmount=excluded.totalAmount,advancePaid=excluded.advancePaid,status=excluded.status,notes=excluded.notes,tripType=excluded.tripType,supplier=excluded.supplier,commission=excluded.commission,takenBy=excluded.takenBy`,
      [b.id, new Date().toISOString(), b.quoteNo ?? '', b.clientName ?? '', b.clientPhone ?? '', b.vehicleName ?? '', b.pickupDate ?? '', b.dropDate ?? '', b.pickupLocation ?? '', b.dropLocation ?? '', b.totalAmount ?? 0, b.advancePaid ?? 0, b.status ?? 'confirmed', b.notes ?? '', b.tripType ?? 'outstation', b.supplier ?? null, b.commission ?? 0, b.takenBy ?? null]
    )
