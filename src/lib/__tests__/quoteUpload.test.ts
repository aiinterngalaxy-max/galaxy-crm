...
vi.mock('../cloudinaryUpload', () => ({
  ...
  CloudinaryUploadError: class extends Error {},
}))
...
beforeEach(() => {
  ...
  uploadToCloudinary.mockResolvedValue({
    url: 'https://res.cloudinary.com/tjez7kaf/quote.pdf', bytes: 1234, publicId: 'q1',
  })
})
...
describe('uploadQuotePdf', () => {
  ...
  // Storage caps files at 10 MB, so anything above it must be shrunk or it is
  // rejected outright. These lock in that behaviour.
  it('compresses a file over the storage cap and uploads the smaller blob', async () => {
    const file = makeFile('big.pdf', 20_000_000)
    const small = new Blob(['small'])
    Object.defineProperty(small, 'size', { value: 1_500_000 })
    compressPdf.mockResolvedValue({
      blob: small, originalBytes: 20_000_000, compressedBytes: 1_500_000, ratio: 0.075, quality: 0.75,
    })

    await uploadQuotePdf({ ...base, file })
    expect(compressPdf).toHaveBeenCalledOnce()
    expect(uploadToCloudinary.mock.calls[0][0]).toBe(small)
  })

  it('takes any shrink at all above the cap, even a small one', async () => {
    const file = makeFile('big.pdf', 20_000_000)
    const barely = new Blob(['x'])
    Object.defineProperty(barely, 'size', { value: 19_000_000 })
    compressPdf.mockResolvedValue({
      blob: barely, originalBytes: 20_000_000, compressedBytes: 19_000_000, ratio: 0.95, quality: 0.3,
    })

    await uploadQuotePdf({ ...base, file })
    // 0.95 would fail shouldUseCompressed, but a rejected upload is worse.
    expect(uploadToCloudinary.mock.calls[0][0]).toBe(barely)
  })

  it('uploads the original when compression cannot process the file', async () => {
    const file = makeFile('weird.pdf', 20_000_000)
    compressPdf.mockResolvedValue(null)

    await uploadQuotePdf({ ...base, file })
    expect(uploadToCloudinary.mock.calls[0][0]).toBe(file)
  })

  it('leaves a small file alone', async () => {
    const file = makeFile('small.pdf', 500_000)
  ...
  it('sanitises the stored filename but keeps the original display name', async () => {
    const result = await uploadQuotePdf({ ...base, file: makeFile('Quote #7 (final).pdf', 100) })

    const sentName = uploadToCloudinary.mock.calls[0][1] as string
    expect(sentName).not.toMatch(/[#()\s]/)
    // Prefixed with the record so the file is traceable in the media library.
    expect(sentName).toMatch(/^leads-L1-\d+-/)
    expect(result.name).toBe('Quote #7 (final).pdf')
  })

  it('reports upload progress through onProgress', async () => {
    uploadToCloudinary.mockImplementation(async (_b: Blob, _n: string, cb: (n: number) => void) => {
      cb(0.25); cb(0.75)
      return { url: 'https://res.cloudinary.com/x/q.pdf', bytes: 100, publicId: 'q' }
    })
  ...
  it('stamps the uploader and a timestamp on the returned doc', async () => {
    ...
    expect(result.url).toBe('https://res.cloudinary.com/tjez7kaf/quote.pdf')
  })
})
...
describe('uploadQuotePdf deduplication', () => {
  ...
  it('records size and hash on the returned doc so future uploads can dedupe', async () => {
    ...
    // size is what storage reports back, not the pre-upload size.
