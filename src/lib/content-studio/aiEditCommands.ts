...
blur { "type":"blur","start":number,"end":number,"strength":number } — WHOLE-FRAME only, strength 1-20. For "blur just the background/person" use background_blur instead.
background_blur { "type":"background_blur","start":number,"end":number,"strength":number } — real ML segmentation, blurs ONLY the background, subject stays sharp. strength 1-20. "portrait blur"/"bokeh behind me".
pixelate { "type":"pixelate","start":number,"end":number,"strength":number } — same whole-frame-only rule as blur.
motion_blur { "type":"motion_blur","start":number,"end":number,"direction":"horizontal"|"vertical","strength":number } — directional streak blur, strength 1-20. "motion blur"/"speed streak".
directional_blur { "type":"directional_blur","start":number,"end":number,"angle":number,"strength":number } — same streak blur at an arbitrary angle (0-360°, 0=horizontal) — use when a specific angle is named.
zoom_blur { "type":"zoom_blur","start":number,"end":number,"strength":number } — burst blur radiating from frame CENTER (framing doesn't actually change — distinct from motionfx's real zoomPunch). strength 1-20.
radial_blur { "type":"radial_blur","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — same burst blur from a chosen point (x/y 0-1, default 0.5/0.5) instead of always center.
spin_blur { "type":"spin_blur","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — rotational/swirl blur around x/y (default 0.5/0.5). strength 1-20.
tiltshift_blur { "type":"tiltshift_blur","start":number,"end":number,"strength":number,"bandY"?:number,"bandHeight"?:number } — miniature-photo look: sharp horizontal band (bandY 0-1 default 0.5, bandHeight 0.05-0.9 default 0.25), blur increasing outward. strength 1-20.
wave { "type":"wave","start":number,"end":number,"strength":number,"axis"?:"horizontal"|"vertical" } — travelling sinusoidal ripple, rows/columns slide (axis default "horizontal"). strength 1-20. "wavy"/"flag wobble".
ripple { "type":"ripple","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — concentric circles from a point x/y (default 0.5/0.5), like a stone in water. Prefer over wave when rings-from-a-spot is implied.
warp { "type":"warp","start":number,"end":number,"strength":number } — smooth whole-frame heat-haze bend on both axes, no center point. "warp it"/"melting"/"drunk camera".
twirl { "type":"twirl","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — spiral swirl around x/y (default 0.5/0.5), fading with distance. "vortex"/"whirlpool" — NOT rotate (rigid turn).
fisheye { "type":"fisheye","start":number,"end":number,"strength":number } — whole-frame barrel lens bulge, frame stays filled. For a LOCAL bulge use bulge instead.
bulge { "type":"bulge","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — localized outward bulge (magnifying-glass) around x/y (default 0.5/0.5).
squeeze { "type":"squeeze","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — inverse of bulge: localized inward pinch. "pinch"/"suck inward".
stretch { "type":"stretch","start":number,"end":number,"strength":number,"axis"?:"horizontal"|"vertical" } — stretches ONE axis (default horizontal), frame size unchanged so edges crop. Distinct from zoom (scales both axes).
lens_distortion { "type":"lens_distortion","start":number,"end":number,"strength":number,"mode":"barrel"|"pincushion" } — correction-scale distortion, barrel bows lines outward, pincushion inward. mode REQUIRED — clarify if direction isn't implied.
spin { "type":"spin","start":number,"end":number,"strength":number,"direction"?:"clockwise"|"counterclockwise" } — WHOLE FRAME continuously rotates (default clockwise), strength 1-20 = speed. Distinct from rotate (instant snap) and spin_blur (smear, frame never turns).
rotation { "type":"rotation","start":number,"end":number,"fromDegrees":number,"toDegrees":number } — single BOUNDED tilt (-360..360), holds each end value outside the window — a dutch-angle move, NOT continuous. Use for specific degrees/one-way turn; spin for open-ended.
bounce { "type":"bounce","start":number,"end":number,"strength":number } — WHOLE FRAME scale-pop with decaying overshoots, like a spring. strength 1-20.
swing { "type":"swing","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — pendulum: rotates one way then back, decaying, around pivot x/y (default 0.5/0.5). Distinct from spin (one direction, never reverses).
mask { "type":"mask","start":number,"end":number,"shape":"circle"|"rect","x"?:number,"y"?:number,"size"?:number,"feather"?:number } — SPOTLIGHT: normal inside, darkened outside. NOT a cutout tool — use chroma_key for that. x/y center (default 0.5,0.5), size 0.05-0.9 default 0.35, feather 0-0.3 default 0.12.
look { "type":"look","start":number,"end":number,"name":"sepia"|"negative"|"tealOrange"|"vintage"|"cinematic"|"hdr"|"colorize"|"duotone"|"oldFilm"|"super8"|"polaroid"|"camcorder","hueDegrees"?:number } — fixed color-grade preset (use "color" for tunable adjustments). "colorize" tints via hueDegrees (0-360, default ~200/blue). Others are one fixed recipe.
glitch { "type":"glitch","start":number,"end":number,"style":"rgbSplit"|"tvNoise"|"screenFlicker"|"vhs"|"scanLines"|"digitalGlitch"|"signalDistortion","strength"?:number } — strength 0-1 default 0.5. rgbSplit=chromatic-aberration shift. tvNoise=static. screenFlicker=brightness pulse. vhs=rgbSplit+noise+desaturation+vignette. scanLines=CRT lines. digitalGlitch=short jittery bursts. signalDistortion=stronger static push.
light { "type":"light","start":number,"end":number,"style":"flash"|"strobe"|"flicker"|"glow"|"bloom"|"lightLeak","strength"?:number } — strength 0-1 default 0.5. flash=quick decaying pulse. strobe=fast on/off. flicker=gentle wobble. glow/bloom=soft brighten-over-itself. lightLeak=warm orange wash.
lens_flare { "type":"lens_flare","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — bright flare with rings strung from source x/y (default 0.8/0.2) through center. Distinct from light's glow/bloom (whole frame, no shape).
sparkle { "type":"sparkle","start":number,"end":number,"strength":number } — small twinkling bright points scattered over frame. "glittery"/"magic dust".
neon_glow { "type":"neon_glow","start":number,"end":number,"strength":number,"color"?:string } — colored glow around the frame's own highlights, tinted by color (CSS string, default neon pink). Use when a HUE is named — light's glow is white/tonal only.
god_rays { "type":"god_rays","start":number,"end":number,"strength":number,"x"?:number,"y"?:number } — diagonal rays from source x/y (default 0.5/0.1). "crepuscular rays"/"sun rays breaking through".
dust { "type":"dust","start":number,"end":number,"strength":number } — specks drifting/swaying over TIME (distinct from color's static grain).
scratches { "type":"scratches","start":number,"end":number,"strength":number } — thin vertical lines flashing on briefly, damaged-film-print look.
film_burn { "type":"film_burn","start":number,"end":number,"strength":number } — warm burn creeping in from the edge, ragged flickering boundary, grows then recedes. Distinct from light's flat lightLeak.
retro_camera { "type":"retro_camera","start":number,"end":number,"strength":number } — viewfinder corner brackets + blinking REC dot. Combine WITH look's "camcorder" (that's only the color grade) for the full old-camcorder package.
rain { "type":"rain","start":number,"end":number,"strength":number } — falling diagonal rain streaks.
snow { "type":"snow","start":number,"end":number,"strength":number } — falling snowflakes, slow drift/sway.
fog { "type":"fog","start":number,"end":number,"strength":number } — whole-frame hazy blurred-white wash. Distinct from color's vignette (edges only, no blur).
frost { "type":"frost","start":number,"end":number,"strength":number } — icy pale-blue blurred border at frame edges, brightening outward — opposite of vignette (which darkens edges).
motionfx { "type":"motionfx","start":number,"end":number,"style":"cameraShake"|"wobble"|"zoomPunch"|"motionTrail"|"speedRamp","strength"?:number } — strength 0-1 default 0.5. cameraShake=sharp decaying jolt. wobble=sustained sway. zoomPunch=quick zoom in/out. motionTrail=ghosted smear. speedRamp=steps through speeds (use "speed" for one constant change). No "freeze frame" exists — must be a clarification, never approximated.
audiofx { "type":"audiofx","style":"equalizer"|"reverb"|"echo"|"distortion"|"bassBoost"|"pitch"|"mono"|"fadeIn"|"fadeOut"|"voiceChanger","strength"?:number,"direction"?:"up"|"down","duration"?:number,"preset"?:"robot"|"chipmunk"|"deep" } — WHOLE-CLIP, no start/end. strength 0-1 default 0.5, unused by mono/voiceChanger. reverb=approximate layered echo, not true convolution. pitch shifts via "direction" (default up) — "deepen voice"→down. fadeIn/fadeOut use "duration" (default 1s). voiceChanger uses a FIXED "preset" (default robot) — prefer it over pitch whenever a CHARACTER is named ("robot voice"/"chipmunk"), pitch for a plain direction.
datamosh { "type":"datamosh","start":number,"end":number,"strength":number } — motion-smeared frame-bleed, an APPROXIMATION (no raw I-frame/GOP access in this build) via blending preceding frames.
auto_color { "type":"auto_color","start":number,"end":number,"strength"?:number } — one-tap FIXED contrast/saturation/sharpen push, strength 0-1 default 0.6. NOT real adaptive white-balance — use "color" when specific knobs are named.
chroma_key { "type":"chroma_key","start":number,"end":number,"keyColor"?:string,"replacementColor"?:string,"strength"?:number } — keys out keyColor (default green "#00ff00"), replaces with solid replacementColor (default black). strength 0-1 default 0.5 = tolerance. THE cutout tool (mask is not) — only a solid replacement color, never a second clip/image.
double_exposure { "type":"double_exposure","start":number,"end":number,"strength":number } — translucent mirrored copy of the frame superimposed on itself (honest single-clip approximation — no second footage source exists).
split_screen { "type":"split_screen","start":number,"end":number } — hard left/right split, right a mirrored copy, visible seam. No strength. Distinct from double_exposure (alpha blend, no seam).
color { "type":"color","start":number,"end":number,"brightness"?:number,"contrast"?:number,"saturation"?:number,"grayscale"?:boolean,"warmth"?:number,"tint"?:number,"vignette"?:number,"exposure"?:number,"highlights"?:number,"shadows"?:number,"sharpness"?:number,"clarity"?:number,"grain"?:number } — at least one field besides start/end/type required. brightness -1..1 = flat offset; exposure -1..1 = multiplicative camera-dial push — exposure for "overexposed/underexposed", brightness for plain "brighter/darker". contrast/saturation 0..3 (1=unchanged). warmth -1(blue)..1(orange); tint -1(magenta)..1(green). highlights/shadows -1..1 lift/crush just bright/dark end. vignette 0..1. sharpness 0..2 = normal sharpen; clarity 0..1 = softer punchy local-contrast. grain 0..1. Map casual wording: "dull/washed out"→lower saturation (~0.4); "vibrant/punchy"→higher (~1.6); "dim"→negative brightness; "brighter"→positive. "moody/cinematic" alone is too vague — ask what specifically.
fade { "type":"fade","direction":"in"|"out","duration":number } — fades to/from black at start/end, duration seconds (default 1).
...
remove_effect { "type":"remove_effect","effectType":"crop"|"zoom"|"pan"|"speed"|"loop"|"blur"|"pixelate"|"color"|"fade"|"rotate"|"flip"|"reverse"|"audio_noise_reduction"|"mask" } — removes the most recently applied hard-baked effect. Never ask for strength/start/end.
...
"Blur the background but keep the person sharp." (real segmentation exists via background_blur) → {"commands":[{"type":"background_blur","start":0,"end":${ctx.durationSec.toFixed(1)},"strength":8}]}
...
"Swirl the first 3 seconds into a vortex." (fades with distance, so twirl not rotate) → {"commands":[{"type":"twirl","start":0,"end":3,"strength":12}]}
...
"Make the video keep spinning for the first 4 seconds." (open-ended, so spin not rotation) → {"commands":[{"type":"spin","start":0,"end":4,"strength":8}]}
"Tilt the frame from 0 to 15 degrees between 2 and 4 seconds." (specific degrees, one-way, so rotation not spin) → {"commands":[{"type":"rotation","start":2,"end":4,"fromDegrees":0,"toDegrees":15}]}
