...
import {
  ...
  applyBlur, applyPixelate, applyColorAdjust, applyVideoFade, applyRotate, applyFlip, applyReverse, applyNoiseReduction, applyMask, applyLook, applyGlitch, applyLight,
...
export function VideoEditWorkspacePage() {
  ...
  async function confirmAiEdit() {
    ...
    setAiProgress(null)
    try {
      type HardBakeType = 'crop' | 'zoom' | 'pan' | 'speed' | 'loop' | 'blur' | 'pixelate' | 'color' | 'fade' | 'rotate' | 'flip' | 'reverse' | 'audio_noise_reduction' | 'mask' | 'look' | 'glitch' | 'light'
      const HARD_BAKE_TYPES: HardBakeType[] = ['crop', 'zoom', 'pan', 'speed', 'loop', 'blur', 'pixelate', 'color', 'fade', 'rotate', 'flip', 'reverse', 'audio_noise_reduction', 'mask', 'look', 'glitch', 'light']
      ...
      if (hardBake.length) {
        ...
        const ordered = [
          ...
          ...hardBake.filter((c) => c.type === 'color' || c.type === 'blur' || c.type === 'pixelate' || c.type === 'fade' || c.type === 'audio_noise_reduction' || c.type === 'mask' || c.type === 'look' || c.type === 'glitch' || c.type === 'light'),
